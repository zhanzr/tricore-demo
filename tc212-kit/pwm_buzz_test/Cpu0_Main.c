/**********************************************************************************************************************
 * \file Cpu0_Main.c
 * \brief PWM passive buzzer test on P10.5 @ 2048 Hz (GTM TOM0_CH2).
...
#define STEP_DELAY_MS   500u
#define HOLD_DELAY_MS   5000u
...
int core0_main(void)
{
    ...
    while (1)
    {
        ...
        IfxStdIf_DPipe_print(&g_ascStandardInterface,
                "pwm_buzz_test: P10.5 TOM0_CH2 2048 Hz, sweep duty 0-100-0, Die = %.2f C\r\n",
                tempC);
        start_dts_measure();
