...
if [[ -n "${BUILD_DIR:-}" ]]; then
    build_dir="$BUILD_DIR/pwm_buzz_test_build"
elif [[ -d /tmp && -w /tmp ]]; then
    build_dir="/tmp/pwm_buzz_test_build"
else
    build_dir="$HOME/.cache/pwm_buzz_test_build"
fi
...
echo "Linking..."
elf="$build_dir/pwm_buzz_test.elf"
"$gcc_bin" -mcpu=tc23xx -T "$proj/Lcf_Gnuc_Tricore_Tc.lsl" -nostdlib \
    -Wl,--gc-sections -Wl,-Map="$build_dir/pwm_buzz_test.map" \
    -o "$elf" "${objs[@]}" -lgcc -lc -lnosys -lgcc

# --- Hex ---
"$objcopy" -O ihex "$elf" "$build_dir/pwm_buzz_test.hex"
echo "HEX: $build_dir/pwm_buzz_test.hex"
echo "SIZE:"
"$size" "$elf"
