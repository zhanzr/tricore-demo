...
if [[ -n "${BUILD_DIR:-}" ]]; then
    build_dir="$BUILD_DIR/coremark_200m_build"
elif [[ -d /tmp && -w /tmp ]]; then
    build_dir="/tmp/coremark_200m_build"
else
    build_dir="$HOME/.cache/coremark_200m_build"
fi
...
include_dirs+=("-I${proj}/coremark_1_0_1/tc234")
...
# --- Compile all sources (shared Libraries + project root + coremark) ---
objs=()
...
echo "Linking..."
elf="$build_dir/coremark_200m.elf"
"$gcc_bin" -mcpu=tc23xx -T "$proj/Lcf_Gnuc_Tricore_Tc.lsl" -nostdlib \
    -Wl,--gc-sections -Wl,-Map="$build_dir/coremark_200m.map" \
    -o "$elf" "${objs[@]}" -lgcc -lc -lnosys -lgcc

# --- Hex ---
"$objcopy" -O ihex "$elf" "$build_dir/coremark_200m.hex"
echo "HEX: $build_dir/coremark_200m.hex"
echo "SIZE:"
"$size" "$elf"
