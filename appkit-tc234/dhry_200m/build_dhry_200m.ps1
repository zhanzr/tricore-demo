...
#       -funroll-loops duplicates loop bodies for faster execution.
$commonFlags = @(
    '-c',
    '-mcpu=tc23xx',
    '-D__HIGHTEC__',
...
$elf = Join-Path $buildDir 'dhry_200m.elf'
& $gcc "-mcpu=tc23xx" "-T" $ldScript "-nostdlib" "-Wl,--gc-sections" "-Wl,-Map=$buildDir\dhry_200m.map" "-o" $elf @objList "-lgcc" "-lc" "-lnosys" "-lgcc" 2>&1 | Select-Object -Last 20
if (-not (Test-Path $elf)) { Write-Error "Link failed"; exit 1 }
Write-Host "Linked: $elf"

# --- Hex ---
& $objcopy -O ihex $elf (Join-Path $buildDir 'dhry_200m.hex') 2>&1
Write-Host "HEX: $(Join-Path $buildDir 'dhry_200m.hex')"
Write-Host "SIZE:"
